...
class _LoginPageState extends State<Login_Page>{
  ...
  static Future<void> validateAndSubmit(context) {
    if(validateAndSave()){
      try{
        ...
        final user =  FirebaseAuth.instance.signInWithEmailAndPassword(email: _email, password: _password);
  ...
  imgLogin(){
    return new Center(
      child: new Image.asset("assets/login.png",width: 80.0,),
    );}
  ...
  TFpassword (){ return new TextFormField(
  ...
  ForgetPass() {return new Container(
    alignment: Alignment(1.0, 0.0),
    padding: EdgeInsets.only(top:10.0,left:20.0),
    child: InkWell(
        child:Text("Forgot Password",style: new TextStyle(color: Colors.green,fontWeight: FontWeight.bold),)
    ),
  );}
            ...
            child: new Column(
              ...
              children: <Widget>[
                ...
                new Text("OR",textAlign: TextAlign.center,
                    style:new TextStyle(fontSize: 15.0,fontWeight: FontWeight.w600)),
                Devider50,
                Container(
                  height: 40.0,
                  child: new Material(
                    ...
                    child: GestureDetector(
                        onTap: () =>_signIn().then((FirebaseUser user)=> print(user.displayName)).catchError((e)=>print(e)),
                        child: Center(
                            ...
                            switch (result.status){
                              case FacebookLoginStatus.loggedIn:{
                                FirebaseAuth.instance.signInWithFacebook(accessToken: result.accessToken.token)
                                    .then((signedInUser){print('Signed in');}).catchError((e)=> print("error $e"));
